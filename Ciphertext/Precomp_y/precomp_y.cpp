...
int getArrInd(int a, int b, int c, int n){
  return -1 + (((n-1)*(n-1)*a) - ((n-1)*a) - ((n-1)*a*(a-1)) + ((a*(a-1))/2) + (((a-1)*(a)*(2*a-1))/6))/2 + (b-a-1)*(n-1) - ((b+a)*(b-a-1))/2 + c - b;
/*First version
  int d =0;
  for (int i=0; i<n; i++){
    for (int j=i+1; j<n; j++){
...
/*Forth version
  int cnt = -1;
  int N=n-1;
  cnt = cnt + ((N*N*a) - (N*a) - (N*a*(a-1)) + ((a*(a-1))/2) + (((a-1)*(a)*(2*a-1))/6))/2;
  cnt = cnt + (b-a-1)*(N) - ((b+a)*(b-a-1))/2;
  cnt = cnt + c - b;
  return cnt;
*/
...
int main(int argc, char* argv[]){
  ...
  */
  ...
  //Pre-calculate determinant of n choose 2 over the entire dataset.
  vector <Ciphertext> cDets;
  c = 0; //temp counter
  for (int i=0; i<dsSize; i++){
    for (int j=i+1; j<dsSize; j++){
      for (int k=j+1; k<dsSize; k++){
        cDets.push_back(Ciphertext());
        cDets[c] = detCompute(evaluator,cDS[i],cDS[j],cDS[k]);
        //Hasrem//cout << "Ciphertext detCompute((" << iDS[i].x << "," << iDS[i].y <<"), (" << iDS[j].x << "," << iDS[j].y << "))" << endl;
        c++;
      }
    }
  }
  cout << "Time taken for precompute " << argv[1] << ", " << argv[2] << ", " << argv[3] << ": " << (double)(clock() - tStart)/CLOCKS_PER_SEC << endl;
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  


  //Decrypt calculated determinant to ensure(verify) the correctness of computation.
  /*
  c = 0; //temp counter
  Plaintext tmpP;
  int d=0;//temp counter
  for (int i=0; i<dsSize; i++){//Calculate n choose 3.
    for (int j=i+1; j<dsSize; j++){
      for(int k=j+1; k<dsSize; k++){
        cout << "Decrypt(det[(" << iDS[i].x << "," << iDS[i].y <<"), (" << iDS[j].x << "," << iDS[j].y << "), (" << iDS[k].x << "," << iDS[k].y << ")]) = ";
        decryptor.decrypt(cDets[c], tmpP);
        cout << encoder.decode_int64(tmpP) << endl;
        c++;
      }
    }
  }
    ...
    for(int i=0; i<dsSize; i++)
      for(int j=i+1; j<dsSize; j++)
        for(int k=j+1; k<dsSize; k++){
          ...
          //Plaintext p;
          //Precomputed determinant:cDets[???]=detCompute(evaluator, cDS[i], cDS[j], cDS[k])
          tmpC1 = cDets[getArrInd(i,j,k, dsSize)];//Shared precomputed det.          
...
//cout << iAlfa << " @@ " << iBeta << " @@ " << iGamma << endl;
      ...
      cout << "Time taken for query with precompute " << argv[1] << ", " << argv[2] << ", " << argv[3] << ": " << (double)(clock() - tStart)/CLOCKS_PER_SEC << endl;
